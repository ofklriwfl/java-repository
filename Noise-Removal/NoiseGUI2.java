...
public class NoiseGUI2 extends JFrame {
	File selected;
	static Runnable gui;
	...
	public static void updateTime() {
		int byteLength = NoiseGUI2.playSound.getValue();
		...
		NoiseGUI2.playMoment.setText(lengthText + "/" + NoiseGUI2.playMoment.getText().substring(NoiseGUI2.playMoment.getText().length() - 8, NoiseGUI2.playMoment.getText().length()));
	}
	...
	public NoiseGUI2() {
		...
		*/
		...
		NoiseGUI2.select = new JFileChooser() {
			...
			public void approveSelection() {
				if (this.getSelectedFile().getName().substring(this.getSelectedFile().getName().length() - 4, this.getSelectedFile().getName().length()).equals(".wav")) {
					...
					*/
					...
					//changeSize(400, 500);
					changeSize(400, 405);
					NoiseGUI2.sliderName.setVisible(true);
					NoiseGUI2.limit.setVisible(true);
					NoiseGUI2.play.setVisible(true);
					...
					*/
					...
					NoiseGUI2.process.setVisible(true);
					NoiseGUI2.save.setVisible(true);
					NoiseGUI2.status.setText(" Ready");
					NoiseGUI2.newButton.setVisible(true);
					NoiseGUI2.status.setVisible(true);
					NoiseGUI2.playMoment.setVisible(true);
					NoiseGUI2.playSound.setVisible(true);
					NoiseGUI2.playSound.setValue(0);
		...
		*/
		NoiseGUI2.select2 = new JFileChooser() {
			public void approveSelection() {
				...
				*/
					if (this.getSelectedFile().exists() == false) {
						...
						NoiseGUI2.save.setVisible(true);
						NoiseGUI2.newButton.setVisible(true);
						NoiseGUI2.status.setText(" Saved");
						NoiseGUI2.status.setVisible(true);
						NoiseGUI2.play.setVisible(true);
						NoiseGUI2.playMoment.setVisible(true);
		...
		*/
		...
		this.play.addMouseListener(new MouseListener() {
			...
			public void mouseClicked(MouseEvent e) {
				...
				//if (NoiseGUI.sound.play == false) {
					...
					NoiseGUI2.thread.bool(true);
					playNow = true;
					NoiseGUI2.sound.play = true;
					sound.playSound(NoiseGUI2.playSound.getValue());
					//playNow = false;
		...
		this.process.addMouseListener(new MouseListener(
			...
			public void mouseClicked(MouseEvent e) {
				...
				*/
				...
				try {
					sound.processSound(Math.abs(Integer.parseInt(NoiseGUI2.limit.getText())));
				} catch (Exception ex) {
					ex.printStackTrace();
					//NoiseGUI.status.setText("Invalid number format");
				}
				NoiseGUI2.status.setText(" Processing complete");
			}
		...
		this.save.addMouseListener(new MouseListener() {
			...
			public void mouseClicked(MouseEvent e) {
				...
				NoiseGUI2.sliderName.setVisible(false);
				NoiseGUI2.limit.setVisible(false);
				NoiseGUI2.play.setVisible(false);
				NoiseGUI2.process.setVisible(false);
				NoiseGUI2.save.setVisible(false);
				NoiseGUI2.newButton.setVisible(false);
				NoiseGUI2.status.setVisible(false);
				NoiseGUI2.playMoment.setVisible(false);
				NoiseGUI2.playSound.setVisible(false);
				NoiseGUI2.select2.setVisible(true);
				NoiseGUI2.select2.setDialogType(JFileChooser.SAVE_DIALOG);
			}
		...
		this.newButton.addMouseListener(new MouseListener() {
			...
			public void mouseClicked(MouseEvent e) {
				...
				NoiseGUI2.sliderName.setText(" Noise floor: ");
				NoiseGUI2.sliderName.setVisible(false);
				//NoiseGUI.limit.setValue(0);
				NoiseGUI2.limit.setText("0");
				NoiseGUI2.limit.setVisible(false);
				NoiseGUI2.play.setVisible(false);
				NoiseGUI2.process.setVisible(false);
				NoiseGUI2.save.setVisible(false);
				...
				NoiseGUI2.playMoment.setVisible(false);
				NoiseGUI2.playSound.setVisible(false);
				NoiseGUI2.select.setVisible(true);
			}
		...
		this.playSound.addMouseListener(new MouseListener() {
			...
			public void mouseReleased(MouseEvent e) {
				...
				*/
				updateTime();
				//if (playNow == true) {
				if (NoiseGUI2.sound.play == true) {
					while (NoiseGUI2.playSound.getValue() % 4 != 0) {
						NoiseGUI2.playSound.setValue(NoiseGUI2.playSound.getValue() - 1);
					}
					sound.playSound(NoiseGUI2.playSound.getValue());
	...
	public static void main(String[] args) {
		...
		gui = new Runnable() {
			public void run() {
				NoiseGUI2 window = new NoiseGUI2();
